...
- Eight regression tests cover printing/reprinting without navigation, exact captured data, no chat on paper, unavailable receipt guards, preserving other actions, escaped guest input and portal cleanup.
- Full frontend suite: 187 tests across 34 files passed. TypeScript and production build passed; existing UI sourcemap warnings are non-fatal.
